  ...
  actions: {
    signup: {
      async handler(ctx) {
        ...
        const { webId }: { webId: string } = await ctx.call('auth.account.create', {
          username,
          email,
          password
        });

        ctx.emit('auth.registered', { webId });
    ...
    login: {
      async handler(ctx) {
        ...
        const accountData: Account = await ctx.call('auth.account.verify', { username, password });
    ...
    resetPassword: {
      async handler(ctx) {
        ...
        const account: Account = await ctx.call('auth.account.findByEmail', { email });
