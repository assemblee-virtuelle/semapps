    ...
    for (const single of singles) {
      try {
        ...
        if (response.status == 403 || response.status == 404 || response.status == 401) {
          ...
          await this.broker.call('ldp.resource.delete', { resourceUri, webId: 'system' }, { meta: { forceMirror: true } });
          ...
          for (let containerUri of containers.map(c => c.container.value)) {
            await this.broker.call('ldp.container.detach', { containerUri, resourceUri, webId: 'system' }, { meta: { forceMirror: true } });
          }
        } else {
