      ...
      '*'(ctx) {
        if (!ctx.meta.dataset) {
          if (ctx.params.groupUri) {
            ...
            if (parts.length > 2) {
              this.logger.warn(`No dataset found when calling ${ctx.action.name}. Using ${parts[2]}`);
              ctx.meta.dataset = parts[2];
            }
          } else if (ctx.params.groupSlug) {
            const parts = ctx.params.groupSlug.split('/');
            if (parts.length > 1) {
              this.logger.warn(`No dataset found when calling ${ctx.action.name}. Using ${parts[1]}`);
