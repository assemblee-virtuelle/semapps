...
const ActivityPubService = {
  ...
  created() {
    const { baseUrl, activitiesPath, collectionsPath, selectActorData, queueServiceUrl, activateTombstones } =
      this.settings;
    ...
    // @ts-expect-error TS(2345): Argument of type '{ mixins: any[]; settings: { pod... Remove this comment to see the full error message
    this.broker.createService({ mixins: [ApiService] });
