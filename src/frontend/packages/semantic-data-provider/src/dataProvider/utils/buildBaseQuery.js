...
const typeQuery = triple(variable('s1'), namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#type'), variable('type'));
...
const buildBaseQuery = (predicates, ontologies) => {
  ...
  if (predicates) {
    ...
    return {
      ...
      where: [
        typeQuery,
        ...baseTriples.map(triple => ({ type: 'optional', patterns: [triple] })),
      ]
    }
  } else {
