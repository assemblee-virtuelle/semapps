...
const buildBaseQuery = (predicates: any, ontologies: any) => {
  ...
  if (predicates) {
    baseTriples = defaultToArray(predicates).map((predicate, i) =>
      rdf.triple(rdf.variable('s1'), rdf.namedNode(getUriFromPrefix(predicate, ontologies)), rdf.variable(`o${i + 1}`))
    );
    return {
  ...
  baseTriples = [rdf.triple(rdf.variable('s1'), rdf.variable('p1'), rdf.variable('o1'))];
  return {
