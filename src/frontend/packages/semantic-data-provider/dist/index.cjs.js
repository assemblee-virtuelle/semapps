...
const $6fcb30f76390d142$var$getUploadsContainerUri = (config, serverKey) => {
  ...
  if (serverKey)
    return (0, $parcel$interopDefault($bkNnK$urljoin))(
      config.dataServers[serverKey].baseUrl,
      config.dataServers[serverKey].uploadsContainer
    );
          ...
          Object.keys(dataServers[dataServerKey].containers[containerKey]).forEach(type => {
            if (types.includes(type))
              dataServers[dataServerKey].containers[containerKey][type].map(path => {
                const containerUri = (0, $parcel$interopDefault($bkNnK$urljoin))(
                  dataServers[containerKey].baseUrl,
                  path
                );
...
const $566b5adde94810fa$var$deleteMethod = config => async (resourceId, params) => {
  ...
  if (params.meta?.filesToDelete)
    await (0, $6fcb30f76390d142$export$2e2bcd8739ae039).delete(params.meta.filesToDelete, config);
  return {
    data: {
      id: params.id
    }
...
const $8c999cc29c8d6a6c$var$fetchContainers = async (
  containers,
  params,
    ...
    httpClient(containerUri)
      .then(async ({ json: json }) => {
        ...
        if (jsonResponse['@context'] !== jsonContext)
          return (0, $parcel$interopDefault($bkNnK$jsonld)).compact(jsonResponse, jsonContext);
        return jsonResponse;
      })
      .then(json => {
...
const $b6ed253f3374f5d4$var$getListMethod =
  ...
  async (resourceId, params = {}) => {
    ...
    if (dataModel.list?.fetchContainer)
      return (0, $8c999cc29c8d6a6c$export$2e2bcd8739ae039)(containers, params, config);
...
const $ceaafb56f75454f0$var$updateMethod = config => async (resourceId, params) => {
  ...
  $6fcb30f76390d142$export$2e2bcd8739ae039).upload(params.data, config, serverKey);
  params.data = updatedRecord;
  await httpClient(`${params.id}`, {
    method: 'PUT',
    body: JSON.stringify({
...
const $3cfb23eead135e3f$var$fetchUserConfig = async config => {
  ...
  if (token) {
    ...
    if (podKey) {
      ...
      config.dataServers[podKey].baseUrl =
        userData['pim:storage'] || (0, $parcel$interopDefault($bkNnK$urljoin))(webId, 'data');
...
const $ff3623bf1421ebcc$var$findCreateContainerWithTypes = (types, createServerKey, dataServers) => {
  ...
  if (containers.length === 0)
    throw new Error(
      `No container found matching with types ${JSON.stringify(types)}. You can set explicitely the create.container property of the resource.`
    );
  ...
  (0, $bkNnK$react.useEffect)(() => {
    if (dataModel && dataServers) {
      if (dataModel.create?.container) {
        ...
        setCreateContainer(
          (0, $ff3623bf1421ebcc$export$2e2bcd8739ae039)(dataModel.types, dataModel.create?.server, dataServers)
        );
      else {
        const defaultServerKey = (0, $8f44b7c15b8b8e1d$export$2e2bcd8739ae039)('default', dataServers);
        setCreateContainer(
          (0, $ff3623bf1421ebcc$export$2e2bcd8739ae039)(dataModel.types, defaultServerKey, dataServers)
        );
      }
    }
...
const $298b78bb7d4a3358$var$useCreateContainerUri = () => {
  ...
  const dataServers = (0, $c9933a88e2acc4da$export$2e2bcd8739ae039)();
  const getContainerUri = (0, $bkNnK$react.useCallback)(
    resourceId => {
