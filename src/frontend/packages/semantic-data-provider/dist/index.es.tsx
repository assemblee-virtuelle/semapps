...
const $c35ffbda15247c32$var$getUploadsContainerUri = (config: any, serverKey: any) => {
  ...
  if (
    ...
    !config.dataServers[serverKey].containers?.find((c: any) => c.binaryResources)
  )
    serverKey = Object.keys(config.dataServers).find(key =>
      config.dataServers[key].containers?.find((c: any) => c.binaryResources)
    );
  if (serverKey)
    return config.dataServers[serverKey].containers?.find((c: any) => c.binaryResources)?.uri; // No server has an uploads container
  else return null;
};
 ...
 */ const $15b841e67a1ba752$var$findContainersWithTypes = (types: any, serverKeys: any, dataServers: any) => {
  ...
  Object.keys(dataServers).forEach(dataServerKey => {
    if (parsedServerKeys.includes(dataServerKey))
      dataServers[dataServerKey].containers?.forEach((container: any) => {
...
const $aba124ea15ea8bc6$var$fetchContainers = async (
  ...
  const fetchPromises = containers.map((container: any) =>
...
const $47d734d7812e6861$var$defaultToArray = (value: any) => (!value ? [] : Array.isArray(value) ? value : [value]);
...
const $865f630cc944e818$var$buildUnionQuery = (queries: any) =>
  queries.map((q: any) => {
    let triples = q.query;
...
const $05a1b4063d50f1b7$var$fetchSparqlEndpoints = async (
  containers: any,
  resourceId: any,
  params: any,
  config: any
 ...
 */ const $3bcb3ff5a72a9185$var$findContainersWithShapeTree = (
  shapeTreeUri: any,
  serverKeys: any,
  dataServers: any
) => {
...
const $36aa010ec46eaf45$var$isURI = (value: any) =>
  ...
  for (const serverKey of Object.keys(newConfig.dataServers))
    if (newConfig.dataServers[serverKey].containers)
      newConfig.dataServers[serverKey].containers = await Promise.all(
        newConfig.dataServers[serverKey].containers?.map(async (container: any) => {
          return {
            ...container,
            // @ts-expect-error TS(2695): Left side of comma operator is unused and has no s... Remove this comment to see the full error message
            types:
...
const $143de1c1f21344e7$var$isURL = (value: any) =>
...
/** @type {(originalConfig: Configuration) => SemanticDataProvider} */ const $243bf28fbb1b868f$var$dataProvider = (
  ...
  return {
    ...
    uploadFile: waitForPrepareConfig(
      (c: any) => (rawFile: any) => (0, $b17c43e3301545ca$export$a5575dbeeffdad98)(rawFile, c)
    ),
    // @ts-expect-error TS(2695): Left side of comma operator is unused and has no s... Remove this comment to see the full error message
    expandTypes: waitForPrepareConfig(
      (c: any) => (types: any) => (0, $36aa010ec46eaf45$export$2e2bcd8739ae039)(types, c.jsonContext)
    ),
  ...
  transformConfig: async (config: any) => {
    ...
    if (token) {
      ...
      if (dataRegistry['interop:hasDataRegistration']) {
        const results = await Promise.all(
          dataRegistry['interop:hasDataRegistration'].map((dataRegistrationUri: any) => {
            // @ts-expect-error TS(2695): Left side of comma operator is unused and has no s... Remove this comment to see the full error message
      ...
      if (typeRegistrations.public.length > 0 || typeRegistrations.private.length > 0) {
        ...
        for (const mode of Object.keys(typeRegistrations))
          for (const typeRegistration of typeRegistrations[mode]) {
            ...
            const containerIndex = newConfig.dataServers.user.containers.findIndex(
              (c: any) => c.path === container.path
            );
            if (containerIndex !== -1)
...
const $7d9911a250866af6$var$findCreateContainerWithTypes = (types: any, createServerKey: any, dataServers: any) => {
  ...
  const matchingContainers = dataServers[createServerKey].containers.filter((container: any) =>
    container.types?.some((t: any) => types.includes(t))
  );
...
const $87656edf926c0f1f$var$compute = (externalLinks: any, record: any) =>
...
const $487567a146508c4e$var$useGetPrefixFromUri = () => {
  ...
  // @ts-expect-error TS(2695): Left side of comma operator is unused and has no s... Remove this comment to see the full error message
  return (0, $fj9kP$useCallback)(
    (uri: any) => (0, $861da9be2c0e62eb$export$2e2bcd8739ae039)(uri, config.ontologies),
    [
