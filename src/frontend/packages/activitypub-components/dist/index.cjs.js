...
var $583VT$semappssemanticdataprovidersrcnotificationChannelssubscribeToUpdates = require('@semapps/semantic-data-provider/src/notificationChannels/subscribeToUpdates');
...
const $decfdd34cc00b80e$var$useOutbox = () => {
  ...
  const fetch = (0, $583VT$react.useCallback)(async () => {
    ...
    const token = localStorage.getItem('token');
    const blankNodesQuery = (0, $583VT$semappssemanticdataprovider.buildBlankNodesQuery)(['as:object']);
    const query = `
...
const $2b75a2f49c9ef165$var$useItemsFromPagesAndNotifications = (pages, notifications, dereferenceItems) => {
  ...
  const currentItems = !pages
    ? []
    : pages.flatMap(p => (0, $3ff23aa25753c478$export$e57ff0f701c44363)(p.orderedItems || p.items));
  const currentAndNew = currentItems.concat(addItems);
  const items = currentAndNew // Filter out removed items.
...
const $2b75a2f49c9ef165$var$useCollection = (predicateOrUrl, options = {}) => {
  ...
  const {
    ...
    errors: itemErrors
  } = $2b75a2f49c9ef165$var$useItemsFromPagesAndNotifications(data.pages, notifications, dereferenceItems);
  ...
  (0, $583VT$react.useEffect)(() => {
    if (liveUpdates)
      ...
      (0, $583VT$semappssemanticdataprovidersrcnotificationChannelssubscribeToUpdates.createWsChannel)(
        dataProvider.fetch,
        collectionUrl
      )
        .then(webSocket => {
  ...
  const allErrors = (0, $3ff23aa25753c478$export$e57ff0f701c44363)(collectionError).concat(
    (0, $3ff23aa25753c478$export$e57ff0f701c44363)(itemErrors)
  );
  return {
    items: items,
...
const $97664763db3c0a46$var$useInbox = () => {
  ...
  const fetch = (0, $583VT$react.useCallback)(
