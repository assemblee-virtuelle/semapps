        ...
        if (delayBeforeRedirect)
          setTimeout(() => {
            ...
            window.location.href = postSignupRedirect
              ? `${postSignupRedirect}?redirect=${encodeURIComponent(redirectTo || '/')}${(0,
                $2dfd781b793256e6$export$2e2bcd8739ae039)(searchParams)}`
              : redirectTo || '/';
            setLoading(false);
          }, delayBeforeRedirect);
        else {
  ...
  (0, $1obPJ$useEffect)(() => {
    if (!isLoading && identity?.id) {
      if (postLoginRedirect)
        navigate(
          `${postLoginRedirect}?redirect=${encodeURIComponent(redirectTo || '/')}${(0,
          $2dfd781b793256e6$export$2e2bcd8739ae039)(searchParams)}`
        );
